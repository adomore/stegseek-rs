...
![体积](https://img.shields.io/badge/二进制-873%20KB-informational.svg)
...
与原版不同(原版依赖 libmcrypt、libmhash、libjpeg、zlib),本移植 **零 C 库依赖** —— 全部纯 Rust,单个 **873 KB** 二进制,开箱即用。
...
| 📦 **零依赖** | 无需 libmcrypt/libmhash/libjpeg/zlib —— 纯 Rust,单文件 873 KB |
...
| 🕵️ **无密码检测** | `--seed` 暴力搜索 RNG 种子以检测/恢复数据(CVE-2021-27211) |
| 🛡️ **健壮** | 损坏/截断的 JPEG 返回干净错误(经 fuzz 加固),绝不 panic |
| 🧩 **steghide 兼容** | `--embed` / `--extract` / `--info` / `--encinfo` 命令与输出对齐 |

## ⚡ 性能

对 `none.jpg` 用 **rockyou 规模的词表(14,344,391 条不命中口令)** 在 16 核机器上做最坏情况全量扫描,best-of-5:
...
> C++ 用 `-O3 -flto` 重编**并不会更快**(与 `-O2` 持平),所以公平的基准就是各发行版实际分发的 `-O2` 二进制。完整方法与数据见 [`BENCHMARK.md`](BENCHMARK.md)。

## 📦 安装 / 构建

**预编译二进制。** 每个版本标签都会发布 x86-64 Linux(gnu + musl)、macOS(Intel + Apple Silicon)与 Windows 构建,各自附带 `.sha256`:
...
**从源码构建。** 最低 Rust 版本为 **Rust 1.75**;详见 [`BUILD-rs.md`](BUILD-rs.md)。
...
常用参数:`-t` 线程数 · `-c/--continue` 命中后继续搜索以恢复多个内嵌文件 · `-q` 隐藏进度 · `-s` 跳过默认猜测 · `-f` 覆盖已存在文件。
...
- **86 项内建测试**,外加针对现编 C++ stegseek 0.6 oracle 的**双向差分**,覆盖全部 18 密码 × 4 格式 × 明文/加密/压缩。
- JPEG 解码器有**确定性 fuzz** 覆盖(穷举替换字节 + 随机损坏),已把多处越界 panic 变为干净错误。
- [`AUDIT.md`](AUDIT.md) 对活体 C++ oracle 复核了兼容性,并驱动了当前修复(损坏 JPEG 干净报错、`--info -p`、口令交互、`--continue`、实时进度、`-r/-g`)。

```bash
cargo test --workspace                          # 快速套件
cargo test -p stegseek-core -- --ignored        # 外加慢速的 seed 全量扫描
STEGSEEK_REF=/path/to/stegseek cargo test       # 启用差分(需自建 C++ oracle)
...
| 文档 | 内容 | 语言 |
|---|---|:---:|
| [`README.md`](README.md) · [`README.en.md`](README.en.md) | 本页 | ZH · EN |
| [`AUDIT.md`](AUDIT.md) | 2026-08-18 对活体 C++ oracle 的审计 | ZH |
| [`BENCHMARK.md`](BENCHMARK.md) | 吞吐方法、数据与复现步骤 | EN |
| [`COMPATIBILITY.md`](COMPATIBILITY.md) | 与 steghide 一致之处,以及全部已知差异 | EN |
| [`BUILD-rs.md`](BUILD-rs.md) | 构建、差分测试、基准 | EN |
| [`CHANGELOG.md`](CHANGELOG.md) | 版本历史 | ZH |
| [`EMBED_NOTES.md`](crates/stegseek-core/src/EMBED_NOTES.md) | 嵌入与失真优化器设计 | EN |
| [`COVERAGE.md`](crates/stegseek-crypto/COVERAGE.md) | 密码 × 模式对 libmcrypt 的覆盖 | EN |

只有这两份 README 同时存在中英文版本,并由 CI 通过 `scripts/lockstep.py` 维持结构锁步。其余文档均为单语言,各自在开头注明了这一点。
